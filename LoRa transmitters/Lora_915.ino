...
void loop()
{
  ...
  display.drawString(width - 60, height, "Gas: " + String(ppm));
  display.display();

//Each weather data is transmitted 3 times before proceeding to the next
//This is to give us a higher chance of getting the correct value

  Serial.println("Sending via LoRa at 915 MHz...");
/*
  for (int i = 0; i <= 1; i++) {
    LoRa.beginPacket();
    LoRa.print("915 MHz");
